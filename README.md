...
2. **Build Job**: GitHub Actions runs Go tests, builds a Docker image for `linux/arm64`, and pushes it to ECR with an immutable short-SHA tag.
...
5. Output the **IAM Role ARN** that needs to be added to GitHub.
...
Once the variable is configured, every push to `main` triggers the CI/CD pipeline automatically:
...
Once deployed, upload an asset to the S3 bucket and access it through the CloudFront URL:
...
# Upload a test file
aws s3 cp ./test-image.png s3://<bucket-name>/test-image.png

# Access it via CloudFront
curl https://<distribution-domain>.cloudfront.net/test-image.png
```
